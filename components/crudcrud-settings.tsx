...
async function migrateData(fromKey: string, toKey: string) {
  try {
    const fromBase = `https://crudcrud.com/api/${fromKey}`;
    const toBase = `https://crudcrud.com/api/${toKey}`;
    
    for (const endpoint of MIGRATE_ENDPOINTS) {
      try {
        // Get data from old key
        const response = await axios.get(`${fromBase}${endpoint}`);
        ...
        for (const item of items) {
          ...
          await axios.post(`${toBase}${endpoint}`, itemData);
        }
...
const CrudCrudSettings = () => {
  ...
  const handleRotateWithMigration = async () => {
    try {
      ...
      const { fromKey, toKey } = result;
      setIsMigrating(true);
      
      // Perform the actual migration
      try {
        await migrateData(fromKey, toKey);
        setMigrationStatus({ 
          status: 'success', 
